...
onresize=function() {
  ...
  ctx.canvas.style.borderRadius=D/2+"px";
}
...
//    this.m=0.6+(0.4-0.8*Math.random());
  ...
  this.randomize();
  this.getX=(t)=>{ 
    return this.r/3*(Math.cos(this.f1*t)+Math.cos(this.f2*t)+Math.cos(this.f3*t));
  }
  this.getY=(t)=>{ 
    return this.r/3*(Math.sin(this.f1*t)+Math.sin(this.f2*t)+Math.sin(this.f3*t));
  }
  this.setPoints=()=>{
    this.points=[];
...
var draw=()=>{
  ...
  for (let i=0; i<curves.length; i++) {
    ...
    ctx.closePath();
    let lw=2+CSIZE*Math.pow(curves[i].r/CSIZE,5);
    ctx.lineWidth=lw;

...
var Brush=function() {
  this.getPath=()=>{
    ...
    let r=this.sr*Math.pow(this.rf*Math.cos(this.k3+t/this.k4)+(1-this.rf)*Math.cos(t/this.k6),2)
            *(Math.cos(Math.PI/2*t/DUR));
    p.moveTo(x,y+r);
    p.arc(x,y,r,TP/4,3*TP/4);
//p.ellipse(x,y,3*r,r,0,TP/4,3*TP/4);
/*
  p.moveTo(x-r,y);
  p.arc(x,y,r,TP/2,3*TP/4);
  p.moveTo(x-r,y);
  p.arc(x,y,r,TP/2,TP/4,true);
*/
    return p;
  }
  this.randomize=()=>{
    //this.k1=0; //TP*Math.random();
    this.k1=-TP/8+TP/4*Math.random();
    this.k2=60+100*Math.random();
    //this.k3=TP/2; //TP*Math.random();
    this.k3=3*TP/8+TP/4*Math.random();
    //this.k4=20+100*Math.random();
this.k4=40+200*Math.random();
    this.ry=10+100*Math.random();
//    this.sr=10+50*Math.random();
    this.sr=20+100*Math.random();
    this.yf=1-Math.pow(0.9*Math.random(),8);
    this.k5=4+30*Math.random();
    this.rf=1-Math.pow(0.9*Math.random(),10);
    this.k6=4+20*Math.random();
  }
  this.randomize();
}

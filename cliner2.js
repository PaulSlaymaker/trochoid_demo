...
var locate=()=>{
  let rot=TP*Math.pow(Math.sin(t/K1),2);
  p1.setAngle();
...
//const dr1k=new DOMMatrix([-0.5,-S6/3,-S6,1/6,3*EDGE/2,S6*EDGE/3]);	// 1/3, flipped-y
//const dr1ka=new DOMMatrix([-0.5,S6/3,-S6,-1/6,3*EDGE/2,S6*EDGE/3]);	// 1/3
//console.log(dr1ka);
//dmy.multiply(dmky.multiply(new DOMMatrix([-0.5,S6,-S6,-0.5,0,0])))
  let l2set=new Path2D();	// dual kite
  //l2set.addPath(p,dr1k);
  //l2set.addPath(p,dr1ka);
...
//var pkite=new Path2D(l2set);
  ...
  let level3=new Path2D(l3set);	// l3set, triple kite, 3 rotations
  level3.addPath(l3set,dm60);
  level3.addPath(l3set,dm120);
...
let dkite=new Path2D(trian);

let trir1=new Path2D();
trir1.addPath(dkite,dm60);

let trir2=new Path2D();
trir2.addPath(dkite,dm120);
//trian.addPath(trian,dm60);
//trian.addPath(tri,dmf);
let triset=new Path2D(dkite);

triset.addPath(trian2);
triset.addPath(trir1);

//let tril3=new Path2D();
triset.addPath(tri,drd2);
